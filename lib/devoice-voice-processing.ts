...
import { buildElevenLabsClonedSpeech, hasElevenLabsVoiceCloneProvider } from "@/lib/elevenlabs-voice-clone";
...
export function shouldUseSpeechProviderForJob(sourceType: string) {
  return sourceType === "voice_clone"
    ? hasElevenLabsVoiceCloneProvider() || hasOpenAiSpeechProvider()
...
async function completeVoiceCloneWithElevenLabs(job: MediaJob) {
  const sample = await resolveVoiceCloneSample(job);
  const result = await buildElevenLabsClonedSpeech({
    text: sourceText(job),
    name: `DeVoice ${job.id}`.slice(0, 80),
    description: "Generated by DeVoice AI Voice Cloning.",
    sample: {
      bytes: sample.bytes,
  ...
  const artifact = await persistGeneratedVoiceArtifact(job, result.audio);
...
export async function completeVoiceJobWithProviderResult(jobId: string) {
  ...
  if (!hasOpenAiSpeechProvider()) {
    if (job.sourceType === "voice_clone" && hasElevenLabsVoiceCloneProvider()) {
      try {
        return await completeVoiceCloneWithElevenLabs(job);
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown ElevenLabs IVC error.";
        console.warn("ElevenLabs voice cloning failed; falling back to DeVoice demo result.", error);
        return completeWithDemoFallback(job, [withoutUndefined({ provider: "ElevenLabs IVC", status: "failed", error: message })]);
      }
    }
  ...
  try {
    if (job.sourceType === "voice_clone" && hasElevenLabsVoiceCloneProvider()) {
      try {
        ...
        const message = error instanceof Error ? error.message : "Unknown ElevenLabs IVC error.";
        console.warn("ElevenLabs voice cloning failed; falling back to OpenAI Speech.", error);
        return completeWithOpenAiSpeech(job, [withoutUndefined({ provider: "ElevenLabs IVC", status: "failed", error: message })]);
      }
    }

    return completeWithOpenAiSpeech(job);
  } catch (error) {
    const failedProvider = job.sourceType === "voice_clone" && hasElevenLabsVoiceCloneProvider() ? "ElevenLabs IVC" : openAiProviderLabel();
